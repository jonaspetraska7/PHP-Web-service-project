...
class Private_messaging_system{
	...
	**/
	private function _usr_user_of_new_message($to,$from,$subject){
		$r = mysql_fetch_object(mysql_query("SELECT vardas,pavarde,usr FROM " . TBL_USERS . " WHERE id = '" . $to . "'"));
		$u = mysql_fetch_object(mysql_query("SELECT vardas,pavarde FROM " . TBL_USERS . " WHERE id = '" . $from . "'"));
