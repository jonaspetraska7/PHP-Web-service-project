...
define("user", "stud"); // your database username 
define("password", "stud"); // your database password
...
define("MY_WEBSITE_usr", "you@example.com"); // your website usr address

class Private_messaging_system{
	
	public function __construct()
	{
		mysqli_connect(server,user,password) or die ("mysqli was unable to connect to a database using provided information!");
		mysqli_select_db(database) or die ("mysqli was unable to select " . database . " database!");
	}
	...
	**/
	public function send_message($to, $message, $subject, $respond = 0){
		...
		if($this->validate_message($message)){
			mysqli_query($query);
			// uncomment this function out if you want to usr a user of a new message
			//$this->_usr_user_of_new_message($to,$from,$subject);
			return TRUE;
		}else{
			return FALSE;
		}
	...
	*	$subject = subject of a message
	**/
	private function _usr_user_of_new_message($to,$from,$subject){
		$r = mysqli_fetch_object(mysqli_query("SELECT vardas,pavarde,usr FROM " . TBL_USERS . " WHERE id = '" . $to . "'"));
		$u = mysqli_fetch_object(mysqli_query("SELECT vardas,pavarde FROM " . TBL_USERS . " WHERE id = '" . $from . "'"));
		$name = $r->vardas . " " . $r->pavarde;
		$uname = $u->vardas . " " . $u->pavarde;
		$to_usr = $r->usr;
		...
		mail($to_usr,MY_WEBSITE_usr,"New message: " . $subject,$body);
	} // END _usr_user_of_new_message
